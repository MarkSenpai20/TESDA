...
SELECT patient_name, age, diagnosis, doctor_name, billing_amount FROM patients WHERE room_type = 'ICU' AND status = 'Critical' ORDER BY billing_amount DESC;
...
SELECT patient_name, age, department, diagnosis, status FROM patients WHERE age BETWEEN 60 AND 75 AND department IN ('Cardiology', 'Oncology', 'Neurology') ORDER BY age DESC;
...
SELECT patient_name, room_type, billing_amount, department FROM patients WHERE billing_amount BETWEEN 100000 AND 250000 AND (room_type = 'Private' OR room_type = 'ICU') ORDER BY billing_amount DESC LIMIT 10;
...
SELECT patient_name, age, gender, diagnosis, billing_amount FROM patients WHERE diagnosis LIKE '%Cancer%' ORDER BY age ASC;
...
SELECT patient_name, admission_date, department, room_type, status FROM patients WHERE admission_date BETWEEN '2025-07-01' AND '2025-09-30' AND (status = 'Discharged' OR status = 'Stable') ORDER BY admission_date DESC LIMIT 15;
